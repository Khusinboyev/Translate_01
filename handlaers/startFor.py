...
async def welcome(message: types.Message):
	...
	try:
		...
		call_inline = InlineKeyboardMarkup()
		call_inline.add(InlineKeyboardButton(text="✅Tilni tanlash", callback_data="✅Tilni tanlash"))
		if await functions.check_on_start(message.chat.id):
			await message.answer(f"Assalomu alaykum <b>{message.from_user.first_name}</b>\n\nTarjimon botga xush kelibsiz, bu bot siz gaplarni tarjima qilishda yordam beradi.\n\n\nTarjima qilishni boshlash uchun bosing👇", reply_markup=call_inline)
		else:
		...
		for row in sql.fetchall():
			id = row[0]
			name += str(id)+" " + "\n"
		await dp.bot.send_message(chat_id=1918760732, text=f"Botda muammo bor. Siz botga ulagan kanallarni tekshirib ko'ring, agar bot botga ulangan kanallarga admin bo'lmasa bot ishlamaydi\n\n\n\nBorga ulangan kanallar: \n{name}")
